...
def test_tracklet_to_dict_from_dict_roundtrip():
    data = local_tracklet_to_dict(_tracklet())
    restored = local_tracklet_from_dict(data)

    assert restored.local_track_id == 5
    assert restored.class_name == "Person"
    assert restored.frame_ids == [0, 1, 2]
    assert restored.trajectory_2d[0] == (0, 1.0, 2.0, 11.0, 22.0)
    np.testing.assert_allclose(restored.center_3d_mean, np.asarray([1.0, 0.0, 0.0]))
