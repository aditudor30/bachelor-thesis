...
from deep_oc_sort_3d.mtmc.candidate_io import (
    read_candidates_csv,
...
def test_candidates_csv_roundtrip(tmp_path):
    path = tmp_path / "candidates.csv"
    write_candidates_csv([_candidate()], path)

    candidates = read_candidates_csv(path)

    assert len(candidates) == 1
    assert candidates[0].candidate_id == "Warehouse_000__Camera_0000__track_7"
    assert candidates[0].trajectory_3d_sampled == []


def test_candidates_jsonl_roundtrip_preserves_trajectory(tmp_path):
    path = tmp_path / "candidates.jsonl"
    write_candidates_jsonl([_candidate()], path)

    candidates = read_candidates_jsonl(path)

    assert len(candidates) == 1
    assert candidates[0].trajectory_3d_sampled[1] == (10, 2.0, 0.0, 0.0)
    assert candidates[0].is_candidate
