...
def test_candidate_id_and_dict_roundtrip():
    candidate = _candidate()
    data = candidate_to_dict(candidate)
    restored = candidate_from_dict(data)

    assert candidate.candidate_id == "Warehouse_000__Camera_0000__track_7"
    assert restored.candidate_id == candidate.candidate_id
    assert restored.is_candidate
    np.testing.assert_allclose(restored.mean_velocity_3d, np.asarray([0.2, 0.0, 0.0]))
