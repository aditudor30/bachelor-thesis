"""Generate raw full-camera pseudo-3D predictions for Step 15G."""
...
def run(args: Any) -> Dict[str, Any]:
    """Run raw full-camera pseudo-3D generation."""
    cfg = _load_yaml(args.config)
    _apply_generation_flags(cfg, args)
    ...
    summary = generate_raw_for_items(items, cfg, show_progress=args.progress)
    print("raw camera files: %s" % summary.get("camera_count"))
    print("raw success rate: %s" % summary.get("success_rate"))
    return summary


def _apply_generation_flags(cfg: Dict[str, Any], args: Any) -> None:
    section = cfg.setdefault("generation", {})
    if args.overwrite:
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate raw full-camera pseudo-3D predictions.")
