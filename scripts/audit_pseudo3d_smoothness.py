"""Audit smoothness of pseudo-3D predictions."""

import argparse
from pathlib import Path
from typing import Any, Dict, List

from deep_oc_sort_3d.audit3d.audit3d_io import iter_data_files, write_csv, write_json
...
def run(args: Any) -> Dict[str, Any]:
    predictions = []
    for path in iter_data_files(args.predictions_root, [".csv"]):
    ...
    write_json(compact, args.output_root / "pseudo3d_smoothness_summary.json")
    write_csv(per_object, args.output_root / "pseudo3d_smoothness_per_track.csv")
    write_csv(worst_pseudo3d_jumps(predictions), args.output_root / "worst_pseudo3d_jumps.csv")
    print("Pseudo3D smoothness objects: %s" % compact.get("object_count"))
    return compact


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Audit pseudo-3D smoothness.")
